...
class SwapAnimation extends Animation{
    ...
    run(){
        ...
        editCounter(false,false);
        pillars[this.i1].setColor(CHANGE_COLOR);
        pillars[this.i2].setColor(CHANGE_COLOR);
        window.setTimeout(() => {
            pillars[this.i1].setHeight(this.val1);
            pillars[this.i2].setHeight(this.val2);              
        }, speed/2);
    }
}
...
document.getElementById("bubbleSortText").addEventListener('click', event => {
    ...
    document.getElementById("bubbleSortText").font = "blue";
});
...
function sortButtonClicked(){
    ...
    //console.log(document.getElementsByTagName("input") );
            //getElementsByClassName("button") );//.style.color = "red";
    ...
    for (let j = 0; j < pillars.length; j++) {
        for (let i = 0; i < length; i++) {
            ...
            if(length!=pillars.length){
                step.push(new ColorAnimation(READY_COLOR,length)); //Ende
            } 
            ...
            if(i>0){ //Verleich
                ...
                if(pillars[i].value<pillars[i-1].value){ //swap        

                    let val1 = pillars[i].value;
                    pillars[i].value = pillars[i-1].value;
                    pillars[i-1].value = val1;            

                    step.push(new SwapAnimation(i,pillars[i].value,i-1,pillars[i-1].value));
                }
...
function split(start, end) {  //Animation color aktuelles Pivot, start, end? HeightChange color delay ? 
    ...
    while (i < j) {
        ...
        if (i < j) { //swap 
            ...
            pillars[j].value = tmp;
            step.push(new SwapAnimation(i,pillars[i].value,j,pillars[j].value));
    ...
    if (pillars[i].value > pivot) { //swap
        ...
        step.push(new SwapAnimation(end,pillars[end].value,i,pillars[i].value));
...
function heapSort(list){
    ...
    animations.push(new AnimationStep(colorTree()));
    ...
    for (let i = list.length-1; i > 0; i--) { //swap than heapify (fix)
        ...
        list[i].value = tmp;
        let step = [
            new SwapAnimation(0,list[0].value,i,list[i].value), 
            new ColorAnimation(READY_COLOR,i)
        ];
        animations.push( new AnimationStep (step));
...
function heapify(list, length, parantIndex) {
    ...
    while (true) {
        ...
        if(largestIndex == parantIndex){ //right order -> stop
            ...
            let tree = colorTree(length);
            while (tree.length!=0) {
                step.push(tree.pop());
            }
            step.push(new SwapAnimation(parantIndex,list[parantIndex].value,largestIndex,list[largestIndex].value));
            animations.push( new AnimationStep (step));
            parantIndex = largestIndex; //proof changed parent
        }
    }
}
function colorTree(length){
    ...
    const color = {
        color: HEAP_COLOR1,
        swap: function (){
            if (this.color == HEAP_COLOR1) {
                this.color = HEAP_COLOR2;
            }else{
                this.color = HEAP_COLOR1;
            }
        }
    };

    let x = 0;
    for (let i = 0; i < length; i++) { //color by tree layers
        x++;
        step.push(new ColorAnimation(color.color,i));
        if ((x) == 2 ** pow) {    
            pow++;
            x = 0;
            color.swap();
